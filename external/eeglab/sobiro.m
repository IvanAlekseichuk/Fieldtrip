% acsobiro() - A. Chickocki's robust Second-Order Blind Identification (SOBIRO) 
...
if n<m   % under assumption of additive white noise and when the number 
  ...
  Q = diag(real(sqrt(1./Dx(1:n))))*Ux(:,1:n)';
   ...
   n = find(Dx>1e-99,1,'last'); % detect the number of sources
   fprintf('acsobiro(): Estimated number of sources is %d\n',n);
   if rank(X)<n
    fprintf('acsobiro(): Notice that the matrix rank is %d\n',rank(X));
   end
   Q = diag(real(sqrt(1./Dx(1:n))))*Ux(:,1:n)';
end
Xb = zeros(size(X));
...
% Estimate the time delayed covariance matrices:
 ...
 for u = 1:m:pn
   ...
   M(:,u:u+m-1) = norm(Rxp,'fro')*Rxp;  % Frobenius norm = sqrt(sum(diag(Rxp'*Rxp)))
 end
